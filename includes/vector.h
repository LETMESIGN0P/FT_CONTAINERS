...
namespace ft
{
	...
	class vector
	{
		...
		void expand (size_t n)
		{
			...
			expand_no_free(n);
			destroy_block(begin_old, storage_end_old - begin_old);
		}

		void expand_no_free (size_t n)
		{
			size_type s = size();
			pointer begin_old = _begin;
		...
		void insert (iterator position, size_type n, const value_type& value)
		{
			if (capacity() < size() + n)
			{
				size_t index_pos = position - begin();
		...
		insert (iterator position, InputIterator first, InputIterator last)
		{
			...
			if (capacity() < size() + n)
			{
				...
				{
					if (capacity() * 2 < capacity() + n)
						expand_no_free(capacity() * 2 + n);
					else
						expand_no_free(capacity() * 2);
				}
		...
		iterator erase (iterator first, iterator last)
		{
			...
			if (n)
			{
				...
				size_type i = end() - last;
				while (i--)
				{
					if (start + n < _end)
						_alloc.construct(start, *(start + n));
					start++;
				}

				clear_block(start + n, end() - last);
				_end -= n;
			}
			return (first);
		}
