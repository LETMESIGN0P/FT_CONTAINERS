...
namespace ft
{
	...
	class map
	{
		public: 
			...
			typedef	ft::pair<key_type, mapped_type>			used_value_type;
		...
		public:
			...
			explicit map (const key_compare& __comp = key_compare(), const allocator_type& alloc = allocator_type()): _alloc(alloc), comp(__comp)
			{
			}

			template <class InputIterator>  
			map (InputIterator first, InputIterator last, const key_compare& __comp = key_compare() ,const allocator_type& alloc = allocator_type()):  _alloc(alloc), comp(__comp)
			{
				insert(first, last); // todo enable if
			}

			map (const map& x): _alloc(x._alloc), comp(x.comp)
			{
			...
			template <class InputIterator>  
			void
			insert (InputIterator first, InputIterator last, typename ft::enable_if<!ft::is_integral<InputIterator>::value, InputIterator>::type* = NULL)
			{
			...
			friend bool
			operator==(const map<_Key, _T, _Compare, _Alloc> & lhs, const map<_Key, _T, _Compare, _Alloc>& rhs);
