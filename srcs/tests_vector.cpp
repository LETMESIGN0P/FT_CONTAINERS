...
bool v_check_content_const (const ft::vector<int> &ft_vec, const std::vector<int> &stl_vec)
{
	...
	if (n && *(ft_vec.rend() - 1) != *(stl_vec.rend() - 1))
	{
		if (ok)
			std::cout << std::endl;
...
bool v_test ()
{
	...
	if (!(ft_vec_const2 <= ft_vec_const) || ft_vec <= ft_vec_def)
	{
	...
	if (ft_vec != eq || ft_vec.size() != eq.size() || ft_vec.max_size() != eq.max_size() || ft_vec.capacity() != eq.capacity() || \
		ft_vec.empty() != eq.empty() || ft_vec.front() != eq.front() || ft_vec.back() != eq.back() || \
		ft_vec.get_allocator() != eq.get_allocator())
	{
		if (state)
			std::cout << std::endl; 
		state = false;
